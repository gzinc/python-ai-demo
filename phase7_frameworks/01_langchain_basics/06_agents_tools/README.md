...
> Use the modern patterns shown above for new projects.
...
# create agent (returns BaseAgent)
...
# create agent (returns executable CompiledStateGraph)
agent = create_agent(model=llm, tools=tools)

# execute directly with message-based API
result = agent.invoke({"messages": [HumanMessage(content="What's 25 * 4?")]})
```

**Why deprecated?**
- `AgentExecutor` adds unnecessary complexity
- LangGraph provides better state management
- Message-based API is more flexible
- Direct invocation is simpler and clearer

### initialize_agent (Legacy)
...
**Old Pattern**:
```python
result = executor.run("What's the weather?")  # returns string
```
...
# returns dict with 'messages' key containing full conversation
```
...
|------------|-------------------|-------|
| `create_react_agent` | `create_agent` (LangGraph) | 1.0+ |
| `initialize_agent` | `create_agent` | 1.0+ |
| `AgentExecutor` | Direct agent invocation | 1.0+ |
| `.run(input)` | `.invoke({"messages": [...]})` | 1.0+ |
| `hub.pull("hwchase17/react")` | Built-in prompting | 1.0+ |
| `verbose=True` | `.stream()` for debugging | 1.0+ |
...
# new
messages = result["messages"]
answer = messages[-1].content  # last message content
```

**Benefits of Migration**:
- ✅ Simpler API (fewer classes to manage)
- ✅ Better state management (LangGraph's automatic tracking)
- ✅ More flexible (message-based interface)
- ✅ Easier debugging (stream intermediate steps)
- ✅ Future-proof (modern LangChain development direction)
